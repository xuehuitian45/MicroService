...
class Evaluator:
    ...
    def evaluate(self):
        ...
        """
        results = []
        for folder in os.listdir(self.data_config.result_path):
            dir_path = os.path.join(self.data_config.result_path, folder)
            if os.path.isdir(dir_path):
                self.partitions = self.load_data(os.path.join(dir_path, "result.json"))
                ...
                report = {
                    "ISDD": self.calculate_ISDD(),
                    "SDE": self.calculate_SDE(),
                    ...
                    "SCDR": self.calculate_service_cycle_ratio()
                }
...
def main(data_config: DataConfig):
    evaluator = Evaluator(data_config)
